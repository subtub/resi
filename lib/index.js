  ...
  process: function(content, beginTag, endTag) {
    ...
    if (lexed.length === 1) {
      if (lexed[0].type === 'text') {
        ...
        tmp = fs.readFileSync(lexed[0].content, 'utf8');
      }
      else if (lexed[0].type === 'url') {
        tmp = execSync('curl -s '+lexed[0].content);
      }
    }
    // If there are more than one object...
      ...
      for (var i=0; i<lexed.length; i++) {
        if (lexed[i].type === 'text') {
          ...
          tmp += fs.readFileSync(lexed[i].content, 'utf8');
        }
        else if (lexed[i].type === 'url') {
          tmp += execSync('curl -s '+lexed[i].content);
        }
      }
    }
