...
Resi.prototype.assembly = function(tokens) {
  ...
  if (tokens.length === 1) {
    ...
    else if (tokens[0].type === CONSTANTS.TOKEN_TYPE.LOG) {
      console.log(CONSTANTS.CONSOLE.BEGIN_LOG+tokens[0].content);
    }
  }
  // If there are more than one object...
    ...
    for (var i=0; i<tokens.length; i++) {
      ...
      else if (tokens[i].type === CONSTANTS.TOKEN_TYPE.LOG) {
      console.log(CONSTANTS.CONSOLE.BEGIN_LOG+tokens[i].content);
    }
    }
  }
