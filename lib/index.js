...
var resi = {
  ...
  readFileSync: function(path, options) {
    ...
    var result = this.compile(data, beginTag, endTag);
    return result;
  },
   ...
   * @returns {String} The compiled result.
   */
  compile: function(content, beginTag, endTag) {
    ...
    if (lexed.length === 1) {
      if (lexed[0].type === 'text') {
        ...
        tmp = this.compile(data, beginTag, endTag);
      }
    ...
    // If there are more than one object...
      ...
      for (var i=0; i<lexed.length; i++) {
        if (lexed[i].type === 'text') {
          ...
          tmp += this.compile(curlData, beginTag, endTag);
        }
