    ...
    // if more than one elements exist, min. one tag is at the content.
      ...
      for (var i=0; i<contentOpenTagArray.length; i++) {
        ...
        if (contentArray.length === 1) {
          if (contentArray[0] !== '') tokens.push( {type: 'text', content: contentArray[0]} );
        }
        // if there is a tag...
        else {
          // check if it is an url, set the type to "url".
          if (this.isHttp(contentArray[0])) {
            tokens.push( {type: 'weburl', content: contentArray[0]} );
          }
          // check if it is a script, set the type to "script".
            ...
            tokens.push( {type: 'script', content: scriptContent[1]} );
          }
          // set the type to "file".
          else {
            tokens.push( {type: 'file', content: contentArray[0]} );
          }
          if (contentArray[1] !== '') tokens.push( {type: 'text', content: contentArray[1]} );
        }
      }
    }
    return tokens;
