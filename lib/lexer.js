...
module.exports = Lexer = {
      ...
      for (var i=0; i<contentOpenTagArray.length; i++) {
        ...
        // if there is a tag...
          ...
          if (this.isHttp(contentArray[0])) {
            tokens.addWeburl(contentArray[0]);
          }
   ...
   */
  isHttp: function(str) {
    var whitespaceless = utils.killWhitespace(str);
    var isHttp = whitespaceless.indexOf('http://');
    var isHttps = whitespaceless.indexOf('https://');
    return (isHttp === 0 || isHttps === 0) ? true : false;
  },
